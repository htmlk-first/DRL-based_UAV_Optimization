...
DDQN 대비 핵심 개선:
1. Actor-Critic 구조
   - Actor  : 상태 → 연속 행동 (정책 네트워크)
   - Critic : (상태, 행동) → Q값 (가치 네트워크)
2. 연속 행동 공간 (Continuous Action Space)
   - tanh 출력으로 [-1, 1] 범위의 행동 생성
3. Soft Target Update (τ-기반 Polyak 평균)
   - DDQN의 hard update (주기적 전체 복사) 대비 안정적
4. Ornstein-Uhlenbeck (OU) Noise
   - 시간 상관 탐색 노이즈 → 물리 시스템에 적합
   - DDQN의 ε-greedy 대비 부드러운 탐색
...
class OUNoise:
    """
    Ornstein-Uhlenbeck 과정 — 시간 상관 탐색 노이즈

    DDQN의 ε-greedy는 무작위로 행동을 뒤바꾸지만,
    OU 노이즈는 이전 노이즈와 상관된 부드러운 노이즈를 생성하여
    연속 행동 공간에서 일관된 방향 탐색을 가능하게 함.
    """
...
class DDPGAgent:
    ...
    def select_action(self, state, add_noise=True):
        """
        Actor 네트워크로 행동 선택 + OU 노이즈

        DDQN: ε 확률로 랜덤 행동 (불연속)
        DDPG: 결정적 정책 + 연속 노이즈 → 부드러운 탐색
        """
    ...
    def _soft_update(self, source, target):
        ...
        DDQN은 N 스텝마다 가중치를 통째로 복사(hard copy)하여
