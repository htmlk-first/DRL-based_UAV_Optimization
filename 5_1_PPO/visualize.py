"""
UAV 환경 시각화 모듈 (라이트 테마, PPO용)
- plot_grid           : 현재 환경 상태
- plot_path           : 비행 경로 + 통계 패널
- plot_reward_curve   : 학습 보상 곡선
- plot_success_curve  : 성공률 곡선
- plot_policy_loss    : Policy 손실 곡선
- plot_value_loss     : Value 손실 곡선
- plot_entropy_curve  : 엔트로피 곡선 (PPO 추가)
...
def plot_policy_loss(losses, window=50,
                     title="PPO Policy Loss", save_path=None):
    """PPO 정책 손실 곡선 (DDPG의 Actor Loss 대응)"""
    fig, ax = plt.subplots(figsize=(12, 4))
    _style_fig(fig)
    _style_ax(ax)
...
def plot_value_loss(losses, window=50,
                    title="PPO Value Loss", save_path=None):
    """PPO 가치 손실 곡선 (DDPG의 Critic Loss 대응)"""
    fig, ax = plt.subplots(figsize=(12, 4))
    _style_fig(fig)
    _style_ax(ax)
...
# ── 3e. plot_entropy_curve (PPO 추가) ─────────────────────────────────────────
def plot_entropy_curve(entropies, window=50,
                       title="PPO Entropy", save_path=None):
