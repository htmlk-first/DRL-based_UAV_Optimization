"""
UAV 3D 환경 시각화 모듈 (라이트 테마, SAC 3D용)
- plot_path_3d         : 3D 비행 경로 + 통계 패널
- plot_reward_curve    : 학습 보상 곡선
- plot_success_curve   : 성공률 곡선
- plot_actor_loss      : Actor 손실 곡선 (SAC)
- plot_critic_loss     : Critic 손실 곡선 (SAC)
...
def _draw_3d_scene(ax, env, trail=None, visited=None):
    ...
    for i, wp in enumerate(env.waypoints):
        ...
        ax.text(wp[0] + 0.3, wp[1] + 0.3, wp[2] + 0.3,
                f"WP{i+1}", fontsize=10, color=clr, fontweight="bold")
...
# ── 1. plot_path_3d ───────────────────────────────────────────────────────────
def plot_path_3d(env, path=None, title="UAV 3D Flight Path (SAC)", save_path=None):
    if path is None:
        path = env.path
    n = len(path)
    ...
    cbar.set_label("Step", color=TEXT_CLR, fontsize=11)
...
def plot_reward_curve(rewards, window=50,
                      title="SAC 3D Training Reward Curve", save_path=None):
...
def plot_success_curve(success_history, window=50,
                       title="SAC 3D Training Success Rate", save_path=None):
    fig, ax = plt.subplots(figsize=(12, 4))
    _style_fig(fig)
    _style_ax(ax)
...
# ── 4. plot_actor_loss (SAC) ──────────────────────────────────────────────────
def plot_actor_loss(losses, window=50,
                    title="SAC 3D Actor Loss", save_path=None):
    """Actor(정책) 손실 곡선"""
    fig, ax = plt.subplots(figsize=(12, 4))
    _style_fig(fig)
    _style_ax(ax)
...
# ── 5. plot_critic_loss (SAC) ─────────────────────────────────────────────────
def plot_critic_loss(losses, window=50,
                     title="SAC 3D Critic Loss", save_path=None):
    """Critic(가치) 손실 곡선"""
    fig, ax = plt.subplots(figsize=(12, 4))
    _style_fig(fig)
    _style_ax(ax)
...
# ── 6. plot_alpha_curve (SAC 전용) ────────────────────────────────────────────
def plot_alpha_curve(alphas, window=50,
                     title="SAC 3D Entropy Coefficient (α)", save_path=None):
...
def make_flight_gif_3d(env, path=None, title="UAV 3D Flight (SAC)",
...
def plot_comparison(results_dict, window=50,
    ...
    ax.set_ylabel(f"Total Reward (Moving Avg, w={window})", fontsize=13)
