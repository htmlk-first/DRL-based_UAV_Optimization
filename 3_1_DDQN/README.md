...
- `3_1_DDQN`은 `2_1_DQN` 대비 **그리드 크기 확장(20x20 -> 30x30)**, **웨이포인트 증가(2개 -> 3개)**, **DDQN 타깃 계산**을 적용한 버전입니다.
...
| 행동 공간 | 이산 4방향 |
| 시작 위치 | `(0, 0)` |
| 웨이포인트 | 3개 (`(10,10)`, `(20,20)`, `(29,29)`), **순서대로 방문** |
| 장애물 | 기본 고정 배치 약 90개 셀 (`size^2 / 10`) |
| 랜덤 장애물 모드 | `num_random_obstacles=15` |
| 이동 비용 | 1.0 |
| 에너지 예산 | 최단 맨해튼 경로 x 2.5 |
| 최대 스텝 | 900 (`30 x 30`) |
