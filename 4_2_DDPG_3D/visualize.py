"""
UAV 3D 환경 시각화 모듈 (라이트 테마, DDPG 3D용)
...
# Fallback for legacy building metadata; current environments provide the
# real footprint in EnvConfig3D so visualization and collision cells match.
...
def plot_critic_loss(losses, window=50,
                     title="DDPG 3D Critic Loss", save_path=None):
    """Critic 네트워크 학습 손실 곡선"""
    fig, ax = plt.subplots(figsize=(12, 4))
    _style_fig(fig)
    _style_ax(ax)
...
def plot_actor_loss(losses, window=50,
                    title="DDPG 3D Actor Loss", save_path=None):
    """Actor 네트워크 학습 손실 곡선"""
    fig, ax = plt.subplots(figsize=(12, 4))
    _style_fig(fig)
    _style_ax(ax)
