...
def _draw_obstacle_cubes(ax, obstacles, alpha=0.25, buildings=None):
    """장애물을 건물(기둥) 형태로 그리기. buildings가 없으면 개별 큐브."""
    ...
    if buildings:
        ...
        for (bx, by), h in buildings.items():
            ...
            for bz in range(h):
                drawn.add((bx, by, bz))
        # buildings에 속하지 않는 나머지 장애물은 개별 큐브로
...
def _draw_building(ax, x, y, height, color, alpha=0.3):
    """(x,y) 위치에 z=0부터 height까지의 건물 기둥 그리기"""
    s = 0.4  # 반 너비
    z_lo = -0.4
    z_hi = height - 0.6
    if z_hi <= z_lo:
    ...
    faces = [
        ...
        [verts[4], verts[5], verts[6], verts[7]],  # top (옥상)
...
def _draw_3d_scene(ax, env, trail=None, visited=None):
    ...
    for i, wp in enumerate(env.waypoints):
        ...
        ax.text(wp[0] + 0.3, wp[1] + 0.3, wp[2] + 0.3,
                f"WP{i+1}", fontsize=7, color=clr, fontweight="bold")
...
def plot_path_3d(env, path=None, title="UAV 3D Flight Path", save_path=None):
    ...
    cbar.set_label("Step", color=TEXT_CLR, fontsize=8)
    ...
    moves = n - 1
    # 에너지 계산: 경로에서 z 이동은 move_cost_z 분리
    energy_used = 0
    for i in range(1, n):
        dz = abs(path[i][2] - path[i-1][2])
        if dz > 0:
            energy_used += env.config.move_cost_z
        else:
            energy_used += env.config.move_cost
    ...
    for label, value, clr in stats:
        ax_info.text(0.08, y, label, transform=ax_info.transAxes,
                     fontsize=7, color=MUTED_CLR, va="top")
        ax_info.text(0.08, y - 0.045, value, transform=ax_info.transAxes,
                     fontsize=11, fontweight="bold", color=clr, va="top")
        ax_info.text(0.08, y - 0.085, "─" * 22, transform=ax_info.transAxes,
                     fontsize=6, color=BORDER_CLR, va="top")
        y -= 0.13
...
# ── 4. plot_loss_curve ────────────────────────────────────────────────────────
def plot_loss_curve(losses, window=100,
...
def make_flight_gif_3d(env, path=None, title="UAV 3D Flight",
    ...
    def draw_frame(step_i):
        ...
        ax.set_title(f"{title}  ·  Step {step_i} / {len(path) - 1}",
                     fontsize=10, fontweight="bold", pad=7, color=TEXT_CLR)
        ...
        moves = step_i
        # 간략 에너지 추정 (경로상 z 이동 반영)
        energy_used = 0
        for i in range(1, min(step_i + 1, len(path))):
            dz = abs(path[i][2] - path[i-1][2])
            energy_used += env.config.move_cost_z if dz > 0 else env.config.move_cost
        ...
        ax_info.text(0.5, by + bh + 0.025, "ENERGY",
                     ha="center", va="bottom", fontsize=7, color=MUTED_CLR,
        ...
        stat_rows = [
            ...
            ("POSITION", f"({cur[0]},{cur[1]},{cur[2]})", TEXT_CLR),
        ...
        for lbl, val, clr in stat_rows:
            ax_info.text(0.5, y, lbl, ha="center", va="top", fontsize=6.5,
        ...
        ax_info.legend(handles=leg_items, loc="lower center",
                       fontsize=6.5, facecolor=DARK_BG,
