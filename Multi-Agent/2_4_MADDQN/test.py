"""MADDQN Test: loads saved model and regenerates result plots."""
...
from maddqn_agent import MADDQNAgent
...
if __name__ == "__main__":
    ...
    model_path = os.path.join(RESULTS, "maddqn_model.pt")
    ...
    try:
        ...
        raise RuntimeError(
            "The saved maddqn_model.pt is incompatible with the current "
            "4-UAV / 10-waypoint observation dimension. Run train.py again "
            "to create a fresh checkpoint."
        ) from exc
    agent.epsilon = 0.0

    rewards, successes, losses = load_log()
    plot_reward_curve(rewards, window=100, title="MADDQN Training Curve",
                      save_path=os.path.join(RESULTS, "training_curve.png"))
    plot_success_curve(successes, window=100, title="MADDQN Success Rate",
                       save_path=os.path.join(RESULTS, "success_curve.png"))
    if losses:
        plot_loss_curve(losses, window=200, title="MADDQN Training Loss",
                        save_path=os.path.join(RESULTS, "loss_curve.png"))

    best_env = evaluate(config, agent, n_episodes=10)
    if best_env is not None:
        plot_path(best_env, title="MADDQN - Best Cooperative Path",
                  save_path=os.path.join(RESULTS, "best_path.png"))
        make_flight_gif(best_env, title="MADDQN Cooperative Flight",
