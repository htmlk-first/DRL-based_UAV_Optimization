"""Train cooperative Multi-Agent Double DQN on the 2D UAV waypoint task."""
...
def train(config, agent, n_episodes=10000, print_every=500, log_path=None):
    ...
    for ep in range(1, n_episodes + 1):
        ...
        while not done:
            ...
            agent.store_batch(obs, actions, rewards, next_obs, done)
            loss = agent.learn()
            if loss is not None:
...
if __name__ == "__main__":
    ...
    agent.save(os.path.join(save_dir, "maddqn_model.pt"))
