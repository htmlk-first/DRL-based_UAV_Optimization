"""MADQN Test: loads saved model and regenerates result plots."""
...
if __name__ == "__main__":
    ...
    model_path = os.path.join(RESULTS, "madqn_model.pt")
    ...
    try:
        ...
        raise RuntimeError(
            "The saved madqn_model.pt is incompatible with the current "
    ...
    plot_reward_curve(rewards, window=100, title="MADQN Training Curve",
                      save_path=os.path.join(RESULTS, "training_curve.png"))
    plot_success_curve(successes, window=100, title="MADQN Success Rate",
                       save_path=os.path.join(RESULTS, "success_curve.png"))
    if losses:
        plot_loss_curve(losses, window=200, title="MADQN Training Loss",
                        save_path=os.path.join(RESULTS, "loss_curve.png"))

    best_env = evaluate(config, agent, n_episodes=10)
    if best_env is not None:
        plot_path(best_env, title="MADQN - Best Cooperative Path",
                  save_path=os.path.join(RESULTS, "best_path.png"))
        make_flight_gif(best_env, title="MADQN Cooperative Flight",
