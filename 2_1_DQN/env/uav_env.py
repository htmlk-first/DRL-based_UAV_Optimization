...
class UAVEnv(gym.Env):
    ...
    State: [x/size, y/size, energy/budget, wp_visited...,
            adj_up, adj_down, adj_left, adj_right,   (0=free, 1=blocked)
            dx_next_wp/size, dy_next_wp/size]
           → 모든 값이 [0, 1] 범위로 정규화됨 (신경망 학습에 유리)
    """
    ...
    def __init__(self, config=None):
        ...
        # 정규화된 상태:
        # [x, y, energy, wp_visited..., adj_4방향, dx_wp, dy_wp]
    ...
    def step(self, action):
        ...
        # Potential-based reward shaping (목표에 가까워지면 보상)
    ...
    def _get_obs(self):
        """정규화된 상태 벡터 반환 (위치 + 에너지 + WP방문 + 인접장애물 + 목표방향)"""
        size = max(self.grid_size - 1, 1)
        x, y = self.uav_pos

        # 인접 셀 장애물 여부 (0=free, 1=blocked)
        adj = []
    ...
    def _next_target(self):
        """다음 미방문 WP 좌표 반환 (모두 방문 시 마지막 WP)"""
        for i, wp in enumerate(self.waypoints):
    ...
    def _potential(self):
        ...
        target = self._next_target()
        return -(abs(self.uav_pos[0] - target[0]) + abs(self.uav_pos[1] - target[1]))
