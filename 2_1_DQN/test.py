"""DQN Test: loads saved model, regenerates result PNGs (no training)."""
import os, sys, csv
import numpy as np
...
if __name__ == "__main__":
    config = EnvConfig(grid_size=20, obstacle_mode="fixed", energy_budget_multiplier=2.0)
    ...
    agent.load(os.path.join(RESULTS, "dqn_model.pt"))
    agent.epsilon = 0.0
    ...
    # ── Path & GIF ──
    if best_env:
        plot_path(best_env, title="DQN - Best Path",
                  save_path=os.path.join(RESULTS, "best_path.png"))
        make_flight_gif(best_env, title="DQN Flight",
